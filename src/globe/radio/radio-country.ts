// Direct port of GEV's src/data/radioCountry.js. ISO 3166-1 alpha-2 country
// code/name normalization for Radio Browser station filtering.
...
export interface NormalizedRadioCountry {
  valid: boolean;
  empty: boolean;
  code: string;
  name: string;
...
export function normalizeRadioCountryInput(
  value: unknown,
): NormalizedRadioCountry {
  if (value === undefined || value === null || value === "") {
    return Object.freeze({ valid: true, empty: true, code: "", name: "" });
  }
  if (
    typeof value !== "string" ||
    value.length > RADIO_COUNTRY_MAX_LENGTH
  ) {
...
export function isRadioCountryCode(value: unknown): boolean {
