/**
 * Flow->road matching: assign TomTom congestion levels to Overpass roads.
 * Ported faithfully from GEV's src/data/flowMatch.js. Pure and Cesium-free.
 *
 ...
 *     bearing agrees within 30 deg (folded mod 180 deg so two-way roads match
...
/** Spatial-hash cell size in meters (~100 m per spec). */
const CELL_SIZE_M = 100;
/** Max snap distance from a road sample to a flow segment. */
const MATCH_RADIUS_M = 35;
/** Max bearing disagreement (degrees, folded mod 180). */
const BEARING_TOLERANCE_DEG = 30;
/** Evenly-spaced samples per road. */
const ROAD_SAMPLES = 7;
/** Minimum matched samples for a road to count as matched. */
const MIN_MATCHED_SAMPLES = 2;

/** A parsed Overpass road object: [[lon, lat], ...] polyline + OSM highway class. */
export interface Road {
  coords: number[][];
  type: string;
}

/** A decoded TomTom flow segment. */
...
/** Match result for a single road (parallel to the input roads array). */
...
/** Bearing disagreement folded mod 180 deg. */
...
/** Squared distance from point (px,py) to segment (ax,ay)-(bx,by), meters^2. */
 ...
 *
 * @returns `matches` is PARALLEL to `roads` (index i describes roads[i]; null =
 *   no flow data for that road, render it exactly as today). `candidateCount`
 *   is the number of roads with at least one flow segment inside the 35 m
 *   search radius regardless of bearing - the denominator for coverage stats.
 */
export function matchFlowToRoads(
  roads: Road[],
  flowSegments: FlowSegment[],
): MatchResult {
  const roadCount = Array.isArray(roads) ? roads.length : 0;
  const matches: (FlowMatch | null)[] = new Array(roadCount).fill(null);
  const empty: MatchResult = { matches, matchedCount: 0, candidateCount: 0 };
  ...
  // over a <=0.05 deg fetch box the distortion is negligible.
  ...
  const projX = (lon: number): number => (lon - refLon) * mPerDegLon;
  const projY = (lat: number): number => (lat - refLat) * M_PER_DEG_LAT;
  ...
  for (const flow of flowSegments) {
    ...
    for (let i = 0; i < coords.length - 1; i++) {
      ...
      // then guarantees a 3x3 cell probe sees everything within 35 m.
  ...
  for (let r = 0; r < roadCount; r++) {
    ...
    if (matchedLevels.length >= Math.max(MIN_MATCHED_SAMPLES, ROAD_SAMPLES / 2)) {
      const med = median(matchedLevels);
      matches[r] = { level: med ?? 0, closure: matchedClosure };
      matchedCount++;
    }
  }
