...
An alternative to God's Eye View (GEV), Velocity, and WorldMonitor for people who want a React + Cesium OSINT globe with server-side caching, TypeScript end to end, and a tactical HUD aesthetic.
...
This project exists because of [Bilawal Sidhu's God's Eye View (GEV)](https://github.com/bilawalsidhu/gods-eye-view), the vanilla-JS 3D globe that went viral on YouTube and later got open-sourced. Full credit to Bilawal for popularizing the format and proving there's an appetite for this kind of live, public-signal OSINT globe.
...
## How Spectre V2 compares to Velocity and WorldMonitor

[Velocity](https://github.com/AndrewCTF/velocity) is a self-hosted, keyless OSINT console (FastAPI + React + Cesium) focused on position history you own, unlimited replay, and server-side correlation via an MCP server. [WorldMonitor](https://github.com/koala73/worldmonitor) is a real-time global intelligence dashboard (globe.gl + deck.gl + Tauri desktop) focused on AI-synthesized news briefs, financial radar, and a Country Instability Index.

| | Spectre V2 | Velocity | WorldMonitor |
|---|---|---|---|
| **Stack** | Next.js 15 + React 19 + Cesium | FastAPI + React + Cesium | Vite + globe.gl + deck.gl, Tauri desktop |
| **Focus** | Tactical OSINT globe, layer toggles | Owned position history, replay, MCP server | AI news aggregation, finance radar, CII |
| **Base map** | Esri World Imagery (keyless) | Keyless Cesium | globe.gl Earth textures |
| **Ships/vessels** | Not yet | Yes (AIS) | Maritime tracking |
| **Civil unrest** | GDELT + ACLED + RSS + social aggregation, instability scoring | Conflict events | AI-synthesized briefs, CII v8, 500+ feeds |
| **Private flights** | VIP tracking, notable people registry | Not available | Not available |
| **Satellite imagery** | Sentinel-2 + NASA GIBS with replay | Not available | Not available |
| **Position history** | Replay timeline (session) | Owned history on disk, unlimited | Not available |
| **AI features** | Not available | MCP server for agent queries | AI briefs, local Ollama, Transformers.js |
| **Desktop app** | No | No | Yes (Tauri 2, macOS/Windows/Linux) |
| **Type safety** | TypeScript | TypeScript | TypeScript |
| **Testing** | Playwright E2E | 2255 tests | Not specified |
| **License** | MIT | AGPL-3.0 | MIT |

### What Velocity and WorldMonitor have that Spectre V2 doesn't (yet)

- **Velocity**: owned position history on disk with unlimited replay, AIS vessel tracking, GPS jamming detection, dark vessel detection (Sentinel-1 SAR), MCP server for AI agent queries, TFR airspace, naval warnings
- **WorldMonitor**: native desktop app (Tauri 2), AI-synthesized news briefs, finance radar (29 stock exchanges, commodities, crypto), Country Instability Index v8, 25 languages with RTL support, 6 site variants from one codebase, 500+ curated news feeds
...
- [God's Eye View (GEV)](https://github.com/bilawalsidhu/gods-eye-view) - Bilawal Sidhu's vanilla-JS 3D OSINT globe. The project that inspired Spectre and popularized the format. Voice control, FLIR/thermal modes, ship tracking, cockpit view.
- [Velocity](https://github.com/AndrewCTF/velocity) - Self-hosted, keyless OSINT situation console (FastAPI + React + Cesium). Owned position history, unlimited replay, AIS vessel tracking, GPS jamming, MCP server for AI agents.
- [WorldMonitor](https://github.com/koala73/worldmonitor) - Real-time global intelligence dashboard (globe.gl + deck.gl + Tauri). AI news aggregation, finance radar, Country Instability Index, 25 languages, native desktop app.

If you're comparing OSINT globes, also check those out. Different trade-offs, different focuses.
